...
import { cleanQuery, isFilterEmpty } from 'utils';
...
export const createConference = (
  ...
  const newConference: any = (conferences as any)[conferenceKey].setIn(
    ['videos'],
    matchedVideos
  );
  return Object.assign({}, newConferences, {
    [`${conferenceKey}`]: newConference
  });
};
...
const computeFilteredConferences = ({
  ...
  const newConferences = Object.keys(conferences).reduce(
    (newConferencesAcc, conferenceKey) => {
      ...
      const matchedVideos =
        ...
        cleanQuery(title).includes(filterValue)
          ? conferenceVideos
...
export const filterEpic: Epic<any, any, ApplicationState> = (action$, store) =>
  action$.ofType(FILTER).pipe(
    ...
    map(action => {
      ...
      const {
        data: { conferences, videosSearchable, presentersSearchable },
