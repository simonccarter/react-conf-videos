...
describe('search', () => {
  ...
  describe('createConference', () => {
    ...
    it('should add array of match video ids to conference', () => {
      ...
      const expectedResult = conferences.setIn([confIdx], {
        videos: [...mVideos]
      });

      // act
      const result = createConference(
        conferences,
        'idx',
        newConferences,
        mVideos
  ...
  describe('filterEpic', () => {
    it('should return the correct action', done => {
      ...
      // act
      filterEpic(action$, mockStore(store), null).subscribe(action => {
        // assert
