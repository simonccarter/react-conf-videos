...
const handler: Handler = async (event, context) => {
  ...
  const searchVideos = async () => {
    ...
    // now get conferences for these videos, replacing their videos object with the calle
    // and get all videos for conferences that matched...
    const queries: string[]= []
    const pipeline = redis.pipeline();
    for(let video of videosAsObjects){
      queries.push(`conference:${video.video.conferenceId}`)
      pipeline.hgetall(`conference:${video.video.conferenceId}`)
    } 
    ...
    for(let video of videosAsObjects){
      const conference = conferenceMap.get(`conference:${video.video.conferenceId}`)
    ...
    // now loop through conferences, putting videos where they belong
    return newResults
  }
  ...
  try {
    // get matching conferences and their videos
    const conferenceResults = await searchConferences()
    ...
    // get conference lists they appear in
    // const conferenceLists = await.client
